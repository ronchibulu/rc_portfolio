...
- **Global scope**: `C:/coding_proj/self_portfolio/.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/coding_proj/self_portfolio/.kilo/notes/`)
...
2. Glob `C:/coding_proj/self_portfolio/.kilo/notes/*.md` (if directory exists) — global notes
...
Global (C:/coding_proj/self_portfolio/.kilo/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/coding_proj/self_portfolio/.kilo/notes/` — works in any directory
