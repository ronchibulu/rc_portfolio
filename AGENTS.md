...
Guidance for Kilo (and other AGENTS.md-aware agents) working in this repository.
...
- Project config lives in `.kilo/` (NOT `.kilocode/` or `.opencode/`).
- Slash commands: `.kilo/command/*.md` (GSD workflow commands already installed).
- Project agents: `.kilo/agents/*.md` (GSD specialist agents already installed).
- Global user config: `C:\Users\user\.config\kilo/` (same structure).
- This file (`AGENTS.md`) is the primary instruction file Kilo reads at session start.

## Workflow

GSD-managed project. Do not skip phases or change scope without using the corresponding slash commands (all resolve to `.kilo/command/gsd-*.md`):
...
- `/gsd-next` — advance to the next logical step (replaces transition)
- `/gsd-complete-milestone` — milestone wrap-up

Full command list is available via `/gsd-help` or by browsing `.kilo/command/`.

## UI/UX

All design decisions run through the `ui-ux-pro-max` skill. Load it with the `skill` tool (or via `/skill ui-ux-pro-max` if registered as a command) before making visual/UX decisions.
